...
function generateMatchPreview(event) {
    const [teamA, teamB] = event.teams;
    const league = event.league;
    const news = REAL_NEWS[Math.floor(Math.random() * REAL_NEWS.length)];
...
async function generate() {
    ...
    for (const event of activeEvents) {
        ...
        const related = activeEvents.filter(e => e.id !== event.id && (e.sport === event.sport || e.league === event.league)).sort((a, b) => b.popularityScore - a.popularityScore).slice(0, 10);
        ...
        await renderPage(path.join(DIST_DIR, event.url, 'index.html'), 'match', {
            ...
            canonical: `${DOMAIN}/${event.url}`, event, related, teamLinks, dateLinks, matchPreview: generateMatchPreview(event), h2h: generateH2H(event), teamNews: REAL_NEWS.slice(0, 3), lastUpdated, criticalCss, schema: generateMatchSchema(event), noindex: false
        });
    ...
    for (const sport of sports) {
        ...
        const filterHtml = renderToString(React.createElement(FilterEngine, {
            initialEvents: activeEvents, initialSport: sport, isHomepage: false, initialDate
        }));
    ...
    const homeFilterHtml = renderToString(React.createElement(FilterEngine, {
        initialEvents: activeEvents, initialSport: 'all', isHomepage: true
    }));
